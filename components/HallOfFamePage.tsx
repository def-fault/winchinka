      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {completedTournaments.map((t) => {
            // 시즌 2, 3, 4인지 확인 (가로 배치를 위해)
            const isFeatured = t.season === '2' || t.season === '3' || t.season === '4';

            return (
              <div
                key={t.id}
                className={`glass-panel p-6 rounded-xl border border-yellow-500/20 relative overflow-hidden group hover:border-yellow-500/50 transition-colors ${isFeatured ? 'md:col-span-2' : ''
                  }`}
              >
